...
class BulkController @Inject() (
  ...
  def post(userId: String) = authAction.async(parse.json(maxLength = 1024 * 10000)) { implicit request =>
  ...
  def getPreviousRequests(userId: String) = authAction.async { _ =>
  ...
  def getResultsSummary(userId: String, uploadReference: String) = authAction.async { _ =>
  ...
  def getCalculationsAsCsv(userId: String, reference: String, csvFilter: CsvFilter) = authAction.async { _ =>
  ...
  def getContributionsAndEarningsAsCsv(userId: String, reference: String) = authAction.async { _ =>
